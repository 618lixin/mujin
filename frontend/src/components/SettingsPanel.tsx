...
import type { AiConfig } from "../features/api/types";
import type {
  AppConfig,
  BackgroundFit,
  ThemeOption,
...
interface SettingsPanelProps {
  ...
  aiConfigStatus: "loading" | "idle" | "saving" | "saved" | "error";
...
export function SettingsPanel({
  ...
  const setAiConfigValue = <Key extends keyof AiConfig>(key: Key, value: AiConfig[Key]) => {
    if (!aiConfig) return;
  ...
  return (
    <aside className="w-[360px] h-full shrink-0 border-l border-paper-deep/30 bg-cloud/92 backdrop-blur-sm flex flex-col">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto scrollbar-hidden px-4 py-4 space-y-5">
        <section className="space-y-2 pb-3 border-b border-paper-deep/25">
          <div className="flex items-center justify-between">
            <label className="block text-[11px] font-body text-ink-faint">AI</label>
            <button
              type="button"
              onClick={() => void (aiConfig ? onSaveAiConfig() : onLoadAiConfig())}
              disabled={aiConfigStatus === "saving" || aiConfigStatus === "loading"}
              className="h-7 px-3 rounded-lg border border-paper-deep/45 text-[11px] text-ink-faint hover:text-bamboo hover:bg-bamboo-mist/50 disabled:opacity-50 transition-colors cursor-pointer"
            >
          ...
          {aiConfig ? (
            <>
              <TextRow
                ...
                onChange={(value) => setAiConfigValue("llmApiKey", value)}
              />
              <TextRow
                label="Base URL"
                value={aiConfig.llmBaseUrl}
                onChange={(value) => setAiConfigValue("llmBaseUrl", value)}
              />
              <TextRow
                label="Model"
                value={aiConfig.llmModel}
                onChange={(value) => setAiConfigValue("llmModel", value)}
              />
              <TextRow
                label="Cheap"
                value={aiConfig.llmCheapModel}
                onChange={(value) => setAiConfigValue("llmCheapModel", value)}
              />
              <ToggleRow
                label="Proactive heartbeat"
                checked={aiConfig.heartbeatProactiveEnabled}
                onChange={(checked) => setAiConfigValue("heartbeatProactiveEnabled", checked)}
              />
              <NumberRow
                label="Heartbeat"
                value={aiConfig.heartbeatIntervalMinutes}
                min={5}
                max={1440}
                onChange={(value) => setAiConfigValue("heartbeatIntervalMinutes", value)}
              />
            </>
          ) : (
            <p className="rounded-lg border border-paper-deep/25 bg-paper-warm/35 px-2.5 py-2 text-[11px] text-ink-ghost">
              {aiConfigStatus === "error" ? "AI config failed" : "Loading AI config"}
            </p>
          )}
          {aiConfigStatus === "error" && aiConfig && (
            <p className="text-[11px] text-red-400">AI config failed</p>
          )}
        </section>
...
function TextRow({ label, value, type = "text", onChange }: TextRowProps) {
  return (
    <label className="flex items-center gap-2 h-9 rounded-lg px-2.5 bg-paper-warm/45 border border-paper-deep/25">
      <span className="w-16 text-[11px] text-ink-faint">{label}</span>
      <input
        type={type}
        value={value}
        onChange={(event) => onChange(event.target.value)}
        className="min-w-0 flex-1 text-[12px] text-ink-soft font-mono"
      />
    </label>
  );
}
  ...
  return (
    <label className="flex items-center gap-2 h-9 rounded-lg px-2.5 bg-paper-warm/45 border border-paper-deep/25">
      ...
      <input
        ...
        className="min-w-0 flex-1 text-[12px] text-ink-soft font-mono"
      />
      <span className="text-[10px] text-ink-ghost">min</span>
    </label>
  );
}
...
function ToggleRow({ label, checked, onChange }: ToggleRowProps) {
  return (
    <button
      ...
      className="flex items-center justify-between h-9 w-full rounded-lg px-2.5 bg-paper-warm/45 border border-paper-deep/25 cursor-pointer"
    >
