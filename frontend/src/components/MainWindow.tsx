...
import { getAiConfig, saveAiConfig } from "../features/api/settings";
...
type AiConfigStatus = "loading" | "idle" | "saving" | "saved" | "error";
...
export function MainWindow({
  ...
  const [activePanel, setActivePanel] = useState<"chat" | "diary" | "memory" | "growth">("diary");
  ...
  const [aiConfigStatus, setAiConfigStatus] = useState<AiConfigStatus>("loading");
  ...
  const loadAiConfig = useCallback(async () => {
    ...
    try {
      const loadedAiConfig = await getAiConfig();
  ...
  const handleAiConfigChange = useCallback((nextConfig: AiConfig) => {
  ...
  const handleGenerateDiary = async () => {
    ...
    try {
      ...
      const diaryNote = loadedNotes.find((n) => n.id === result.noteId);
      if (diaryNote) {
        await handleSelectNote(diaryNote.id);
      }
  ...
  return (
    <div className="w-full h-screen flex flex-col">
      <div className="relative noise-bg bg-cloud overflow-hidden flex flex-col flex-1">
        ...
        {/* Navigation tabs */}
        <div className="relative z-10 flex items-center gap-1 px-5 h-9 bg-paper/30 border-b border-paper-deep/20 shrink-0 select-none">
          {(["chat", "diary", "memory", "growth"] as const).map((panel) => {
            const labels = { chat: "对话", diary: "日记", memory: "记忆", growth: "成长" };
            const isActive = activePanel === panel;
            return (
              <button
                key={panel}
                onClick={() => setActivePanel(panel)}
                className={`px-3 h-7 rounded-lg text-[12px] font-body transition-all cursor-pointer ${
                  isActive
                    ? "bg-bamboo-mist/60 text-bamboo font-medium"
                    : "text-ink-ghost hover:text-ink-faint hover:bg-paper-warm/60"
                }`}
              >
                {labels[panel]}
              </button>
        ...
        <div className="relative z-10 flex flex-1 min-h-0">
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="flex-1 flex flex-col min-w-0">
              ...
              {activePanel === "growth" && <GrowthPanel />}
            </div>
          )}
