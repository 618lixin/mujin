...
import { quickExtract } from "../features/api/diary";
import {
  DEFAULT_TILE_COLOR,
  normalizeTileColor,
  resolveTileColor,
...
export function NotePad({
  ...
  const [aiExtractStatus, setAiExtractStatus] = useState<string | null>(null);
  ...
  const extractSavedNote = useCallback(async (note: Note) => {
    if (!note.content.trim()) {
      setAiExtractStatus("AI extract skipped");
      return;
    }

    setAiExtractStatus("AI extracting");
    try {
      const result = await quickExtract(note.id, note.title, note.content);
      setAiExtractStatus(result.extracted ? "AI extracted event" : "AI extract skipped");
    } catch {
      setAiExtractStatus("AI extract failed");
    }
  }, []);
